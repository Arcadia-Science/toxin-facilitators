...
def main():
    ...
    for filename in os.listdir(args.input_dir):
        ...
        if not filename.endswith(".fasta"):
            continue
        # remove directory path and extension so just have species name and accession
        species_name = os.path.splitext(os.path.basename(filename))[0]
        # open fasta file and modify the headers
        with open(os.path.join(args.input_dir, filename), "r") as fasta_file:
            for record in SeqIO.parse(fasta_file, "fasta"):
                record.id = f"{species_name}_id_{record.id}"
